...
私有 installed CLI 当前没有 upstream 的 MQL 查询命令。默认搜索请使用：

- `meegle workitem search-filter`：按名称 / 人员等常用条件搜索
- `meegle workitem search-by-params`：按字段条件搜索

本文件保留为 MQL 语法背景资料；只有当 live CLI 后续通过 `meegle inspect` 暴露 MQL 搜索能力时，才把这里的语法用于实际命令。

> **重要：MQL 的 `mql` 参数必须是完整的 SQL 查询语句**
