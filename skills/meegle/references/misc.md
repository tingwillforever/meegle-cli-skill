...
默认写评论用 `comment add`，更新已有评论用 `comment update`；查询评论用 `comment list`。删除评论仅在用户明确要求时才可用 `comment remove`，并且必须先提示“删除后不可恢复”；`comment remove` 是 destructive 命令，必须带 `--confirm` 才能执行。
